...
void MD5::decode(dword output[], const byte input[], size_t len)
{
...
void MD5::encode(byte output[], const dword input[], size_t len)
{
...
void MD5::transform(const byte block[block_sz])
{
...
MD5& MD5::finalize()
{
	...
	if (!finalized)
	{
		...
		// Zeroize sensitive information.
		// We can skip this since we are only using this library for demonstration purposes.
...
MD5::operator std::string() const
{
	...
	for (int i = 0; i < 16; i++) 
		sprintf(buf + i * 2, "%02hhx", hash[i]);

	return std::string(buf);
}

void MD5::byte_hash(const char *string_hash, char *byte_hash)
{
	char buf[3];
	buf[2] = 0;
	for (int i = 0; i < 16; i++) 
	{
		buf[0] = string_hash[i * 2];
		buf[1] = string_hash[i * 2 + 1];
		// (char)strtol(buf, NULL, 16);
		std::sscanf(buf, "%02hhx", &byte_hash[i]);
		// printf("%02x", byte_hash[i]);
	}
	// printf("\n");
	// char buf[3];
	// buf[2] = 0;
	// for (int i = 0; i < 16; i++)
	// {
	// 	byte_hash[i] = string_hash[i * 2] * 2 + string_hash[i * 2 + 1];
	// 	// byte_hash[i] = (char)strtol(string_hash + i * 2, NULL, 16);
	// 	// buf[0] = string_hash[i * 2];
	// 	// buf[1] = string_hash[i * 2 + 1];
	// 	// buf[2] = 0;
	// 	// byte_hash[i] = (char)strtol(buf, NULL, 16);
	// }
}

void MD5::str_hash(const char *byte_hash, char *str_hash)
{
	for (int i = 0; i < 16; i++)
	{
		sprintf(str_hash + i * 2, "%02hhx", byte_hash[i]);
	}
}